...
def generate_inferance_engine(data, force_rewrite=False):
    ...
    try:
        ...
        faiss_storage = FaissEmbeddingStorage(
            data_dir=data, dimension=embedded_dimension
        )
...
def chatbot(query, chat_history, session_id):
    ...
    for node in response.source_nodes:
        ...
        # у каждого документа должен быть filename
...
def on_dataset_path_updated_handler(source, new_directory, video_count, session_id):
    ...
    if source == 'directory':
        ...
        generate_inferance_engine(data_dir)
        print("All videos transcripts are downloaded and processed") if status else print(
